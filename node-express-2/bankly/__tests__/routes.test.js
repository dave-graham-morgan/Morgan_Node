...
describe("POST /auth/login", function() {
  ...
  //tests bug 1 
  test("should disallow an incorrect username/password", async function(){
    const response = await request(app)
       .post('/auth/login')
       .send({
        username:'u1',
...
describe("PATCH /users/[username]", function() {
  ...
  test('should allow access if right user', async function(){
    const response = await request(app)
      .patch("/users/u2")
      .send({ _token: tokens.u2, first_name:"new-fn4u2" }) //correct user
    expect(response.statusCode).toBe(200);
    expect(response.body.user.first_name).toEqual("new-fn4u2");
  })
  //test for bug 3
  test('should not allow empty payload', async function(){
    const response = await request(app)
      .patch("/users/u2")
      .send({ _token: tokens.u2 }) //correct user
    expect(response.statusCode).toBe(400);
  })
  //test for bug 6 now working
