...
function sqlForPartialUpdate(table, items, key, id) {
  ...
  let idx = 1;
  let columns = [];
  //fixes bug 3: this was causing an error when there was an empty payload(apart from the _token that is)
  if (Object.keys(items).length === 0) throw new ExpressError("No data to update", 400)
